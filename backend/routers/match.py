...
def get_match_detail(match_id: int, db: Session = Depends(get_db), current_user=Depends(get_current_user)):
    ...
    if current_user.id not in [match.parent_id, match.sitter_id]:
        raise HTTPException(status_code=403, detail="조회 권한이 없습니다.")

    survey = db.query(models.UserSurvey).filter(models.UserSurvey.user_id == match.parent_id).first()
    
    children_info = "정보 없음"
    if survey and survey.children:
        children_list = [f"{c.child_age}세 {c.child_gender}" for c in survey.children]
        children_info = ", ".join(children_list)

    return {
        "id": match.id,
        "parent_name": match.parent.name,
        "location": match.parent.address,
        "start_time": "시간 협의", 
        "duration": "협의", 
        "hourly_pay": survey.hope_pay if survey else 0,
        "children_info": children_info,
        "requirements": survey.warning if survey else "요청사항 없음",
        "status": match.status
    }
